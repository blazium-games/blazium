...
class ClassStatus:
    ...
    def make_output(self) -> dict[str, str]:
        ...
        if self.name.startswith("Total"):
            output["url"] = color("url", "https://docs.blazium.app/en/latest/classes/")
            if flags["s"]:
                output["comment"] = color("part_good", "ALL OK")
        else:
            output["url"] = color(
                "url", "https://docs.blazium.app/en/latest/classes/class_{name}.html".format(name=self.name.lower())
            )
